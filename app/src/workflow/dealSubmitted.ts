...
// credit to buyers") and, because it never places a supplier order until the buyer's
// advance is received (§4, "This eliminates inventory risk"), it never carries its own
// inventory exposure either — so "credit" and "inventory" never apply here. Only
// "supplier" (the confirmed purchase order) is required; "logistics" is added by the
// future B2C workflow only when CommitOS books third-party freight itself, mirroring
// how the `requiredDomains` derivation below (not REQUIRED_BASE_DOMAINS itself, which
// is static) conditionally adds "supplier" on a shortfall. Not yet consumed by any
// workflow — that's the next plan (the actual B2C evaluate/route flow).
